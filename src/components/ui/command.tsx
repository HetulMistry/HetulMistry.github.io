...
function CommandDialog({
  ...
  return (
    <Dialog {...props}>
      ...
      <DialogContent
        className={cn(
          "top-1/3 translate-y-0 overflow-hidden rounded-xl! p-0 bg-transparent border-0 shadow-none ring-0",
          className,
        )}
    ...
    <div data-slot="command-input-wrapper" className="p-2 pb-1">
      <InputGroup className="h-10! rounded-lg! border-white/10 bg-white/4 shadow-none! *:data-[slot=input-group-addon]:pl-3! focus-within:border-white/20 focus-within:bg-white/6">
        <CommandPrimitive.Input
          ...
          className={cn(
            "w-full text-sm text-slate-100 placeholder:text-slate-500 outline-hidden disabled:cursor-not-allowed disabled:opacity-50",
            className,
          )}
          {...props}
        />
...
function CommandItem({
  ...
  return (
    <CommandPrimitive.Item
      ...
      className={cn(
        "group/command-item relative flex cursor-default items-center gap-2 rounded-sm px-2 py-1.5 text-sm outline-hidden select-none in-data-[slot=dialog-content]:rounded-lg! data-[disabled=true]:pointer-events-none data-[disabled=true]:opacity-50 data-selected:bg-white/10 data-selected:text-white transition-colors duration-100 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*='size-'])]:size-4 data-selected:*:[svg]:text-white",
        className,
      )}
      {...props}
    >
