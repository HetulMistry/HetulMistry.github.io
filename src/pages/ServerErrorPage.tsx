...
export default function ServerErrorPage() {
  ...
  const [errorMessage, setErrorMessage] = useState(() => {
    ...
    return "Unhandled exception in application handler";
  });
  const [errorStack, setErrorStack] = useState(() => {
    ...
    try {
      ...
      if (stored) {
        const parsed = JSON.parse(stored);
        if (parsed.stack) return parsed.stack;
      }
    } catch {
      // ignore
    }
  ...
  useEffect(() => {
    document.title = "500 - Internal Server Error | Hetul Mistry";
    ...
    if (msgParam) {
      setTimeout(() => {
        ...
        if (stackParam) setErrorStack(stackParam);
      }, 0);
    } else {
      try {
        const stored = sessionStorage.getItem("lastError");
        ...
        if (stored) {
          ...
          setTimeout(() => {
            ...
            if (parsed.stack) setErrorStack(parsed.stack);
          }, 0);
        }
      } catch {
        ...
        <div className="page">
          <main className="hero">
            <div className="hero-inner">
              ...
              <h1 className="headline">Internal Server Error</h1>
              <p className="subtext">
                Something went wrong on my server. This could be a temporary
                glitch or an unexpected condition. The error has been logged.
              </p>
              <Terminal
                className="terminal"
                startOnView={false}
              ...
              >
                ...
                {errorStack && (
                  <AnimatedSpan
                    className="terminal-line text-xs font-mono opacity-50 whitespace-pre overflow-x-auto max-w-full block py-1 border-t border-b border-white/5 my-2"
                    delay={800}
                  >
                    <span>{errorStack.split("\n").slice(0, 3).join("\n")}</span>
                  </AnimatedSpan>
                )}
                <AnimatedSpan
                  className="terminal-line warning"
                  delay={errorStack ? 1000 : 900}
                >
                  <span>⚠ Stack trace recorded for debugging</span>
                </AnimatedSpan>
                <AnimatedSpan
                  className="terminal-divider"
                  delay={errorStack ? 1100 : 1000}
                >
                  <div className="h-px bg-white/8 my-4" />
                </AnimatedSpan>
                <AnimatedSpan
                  className="terminal-line terminal-muted"
                  delay={errorStack ? 1200 : 1100}
                >
                  What to do
                </AnimatedSpan>
                <AnimatedSpan
                  className="terminal-line"
                  delay={errorStack ? 1300 : 1200}
                >
                  → Check your connection and try again
                </AnimatedSpan>
                <AnimatedSpan
                  className="terminal-line"
                  delay={errorStack ? 1400 : 1300}
                >
                  → If the issue persists, contact me
                </AnimatedSpan>
                <AnimatedSpan
                  className="terminal-divider"
                  delay={errorStack ? 1500 : 1400}
                >
                  <div className="h-px bg-white/8 my-4" />
                </AnimatedSpan>
                <AnimatedSpan
                  className="terminal-links"
                  delay={errorStack ? 1600 : 1500}
                >
